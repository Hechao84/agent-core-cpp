...
echo "  --server       Enable HTTP REST API server + configured channels (default host:port 127.0.0.1:8080)"
echo "  --port <N>     Set server port (default: 8080)"
echo "  --host <IP>    Set server host (default: 127.0.0.1)"
echo "  --no-cli       Disable CLI mode (daemon mode)"
echo "  --help         Show this help message"
echo ""
echo "Example (CLI + HTTP server):"
echo "  LD_LIBRARY_PATH=$SCRIPT_DIR/libs:$SCRIPT_DIR/dist/linux ./dist/linux/jiuwenClaw --server --port 8080 --host 127.0.0.1"
